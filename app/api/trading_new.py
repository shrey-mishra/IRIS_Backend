...
def generate_market_signal(indicators: Dict[str, float]) -> Dict[str, Any]:
...
def get_market_signals(symbol: str = "BTC/USDT", current_user_email: str = Depends(get_current_user)):
    ...
    try:
        # Use public Binance instance for market data (no API credentials needed)
        exchange = binance()
        ...
        print(f"DEBUG: Market signals failed for {symbol}: {e}")
...
def get_all_market_signals(current_user_email: str = Depends(get_current_user)):
    ...
    try:
        ...
        print(f"DEBUG: Generating market signals for {len(supported_symbols)} symbols")
        ...
        for symbol in supported_symbols:
            try:
                ...
                print(f"DEBUG: {symbol} signal: {signal['signal']} (strength: {signal['strength']})")
            except Exception as e:
                print(f"DEBUG: Failed to get signal for {symbol}: {e}")
        ...
        print(f"DEBUG: Generated {len(all_signals)} market signals")
        return all_signals
    except Exception as e:
        print(f"DEBUG: Failed to generate all market signals: {e}")
...
def execute_ai_auto_trade(
    ...
    try:
        ...
        base_trade_size = 5.0  # Minimum $5 trade
        risk_multiplier = risk_level / 5.0
        trade_size_usd = base_trade_size * risk_multiplier * (max_trade_size / 100.0)
        
        # Ensure minimum trade size
        if trade_size_usd < 5.0:
            trade_size_usd = 5.0
        ...
        # Execute the trade using direct trade endpoint
        try:
            ...
            print(f"DEBUG: AI trade execution failed: {trade_error}")
            return {
                "message": f"AI auto trade failed: {str(trade_error)}",
                "signal": signal,
                "strength": strength,
                "trade_size": trade_size_usd,
                "risk_level": risk_level
        ...
        print(f"DEBUG: AI auto trade failed: {str(e)}")
